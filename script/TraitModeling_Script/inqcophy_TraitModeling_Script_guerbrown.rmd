...
cols2 <- setNames(c("blue", "red", "yellow"),
	c("Asexual", "Sexual"))
par(fg="black")
cynipgen <- setNames(data$cynip_generation, data$Synergus)
xx <- strsplit(as.character(cynipgen), split = "+", fixed = T)
pp <- matrix(0, length(cynipgen), 3, dimnames = list(names(cynipgen), c("Asexual", "Sexual", "Asexual+Sexual")))
for (i in 1:nrow(pp)) pp[i,xx[[i]]]<-1/length(xx[[i]])
tiplabels(pie=pp, piecol=cols2,cex=0.15)
par(fg="black")
piecol<-cols2
for(i in 1:ncol(asr2)){
	nn<-strsplit(colnames(asr2)[i],"+",fixed=TRUE)[[1]]
