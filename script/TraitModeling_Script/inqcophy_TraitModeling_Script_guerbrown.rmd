...
```{r oak_section POLYMORPHIC, fig.height=10, fig.width=12.5, fig.align='center'}
...
oak_sectioncols <- setNames(c("firebrick1", "grey75", "dodgerblue"),
...
# Since our states are technically polymorphic, we need to examine our intentions. We have the data for a combination approach but it would be just as informational to examine a non-combination approach to a polymorphic trait reconstruction. So, that's what we'll do. To accomplish this, specify a column without combination traits and add a proportional extraction from other columns that include the original trait to the column. 
oak_sectionasr[,1] <- oak_sectionasr[,1] + 1/2*(oak_sectionasr[,2]) + 1/3*(oak_sectionasr[,3]) + 1/2*(oak_sectionasr[,4])
oak_sectionasr[,5] <- oak_sectionasr[,5] + 1/2*(oak_sectionasr[,2]) + 1/3*(oak_sectionasr[,3]) + 1/2*(oak_sectionasr[,6])
oak_sectionasr[,7] <- oak_sectionasr[,7] + 1/3*(oak_sectionasr[,3]) + 1/2*(oak_sectionasr[,4]) + 1/2*(oak_sectionasr[,6])
oak_sectionasr <- data.frame(oak_sectionasr[,1], oak_sectionasr[,5], oak_sectionasr[,7])
