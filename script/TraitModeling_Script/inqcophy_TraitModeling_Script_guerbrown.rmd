...
oak_sectioncols <- c("red", "blue", "green", "black")
...
oak_sectionpp <- matrix(0, length(oak_section_DATA), 3, dimnames = list(names(oak_section_DATA), c("Quercus", "Lobatae","Virentes")))
...
par(fg = "black")
tiplabels(pie = to.matrix(oak_section, sort(unique(oak_section))),piecol = oak_sectioncols, cex = 0.15)
nodelabels( node = 1:S$Nnode+Ntip(S),
    pie = oak_section.ER$lik.anc, piecol = oak_sectioncols, cex=0.25)
tiplabels(pie = to.matrix(oak_section, sort(unique(oak_section))), piecol = oak_sectioncols, cex = 0.15)
...
```{r cynip_generation POLYMORPHIC, fig.height=10, fig.width=20, fig.align='center'}
...
cynip_generationcols <- setNames(c("blue", "red", "yellow"),
	c("Asexual","Sexual"))
par(fg="black")
...
tiplabels(pie = cynip_generationpp, piecol = cynip_generationcols, cex=0.15)
par(fg="black")
...
par(fg="transparent")
nodelabels(pie = cynip_generationasr, piecol = cynip_generationcols,cex=0.25)
par(fg="black")
legend("left", c("Asexual", "Sexual", "Asexual+Sexual"), pch = 21, pt.bg = c("blue", "red", "yellow"), pt.cex = 3, title = "Gall Former Generation", cex = 1.5)
...
```{r mono.polythalamous POLYMORPHIC, fig.height=10, fig.width=20, fig.align='center'}
...
svg("/home/guerbrown/github_local/biol-4386-course-project-guerbrown/output/inqcophy_traitmodeling_monopolythalamous_guerbrown.svg", width = 20, height = 10)

mono.polythalamouspp <- matrix(0, length(mono.polythalamous), 3, dimnames = list(names(mono.polythalamous), c("0" ,"1", "1+0")))
plot.phylo(S, type = "fan", cex = 2, align.tip.label = F, lwd = 3.5, label.offset = 0.1, use.edge.length = F, open.angle =  165)
...
mono.polythalamouscols <- setNames(c("blue", "red"),
	c("0", "1"))
par(fg="black")
...
tiplabels(pie = mono.polythalamouspp, piecol = mono.polythalamouscols, cex=0.15)
par(fg="black")
...
for(i in 1:ncol(mono.polythalamousasr)){
	...
	if(length(mono.polythalamousnn)==1) piecol[i]<-cols[mono.polythalamousnn]
	else if(length(mono.polythalamousnn)==2) piecol[i] <- colorRampPalette(cols[mono.polythalamousnn])(3)[2]
	else piecol[i]<-"black"
}
names(piecol)<-colnames(mono.polythalamousasr)
par(fg="transparent")
nodelabels(pie = mono.polythalamousasr, piecol = mono.polythalamouscols,cex=0.25)
par(fg="black")
legend("left", c("Monothalamous", "Polythalamous"), pch = 21, pt.bg = c("blue", "red"), pt.cex = 3, title = "Gall Former Tissue", cex = 1.5)
...
```{r gall_size POLYMORPHIC, fig.height=10, fig.width=20, fig.align='center'}
...
gall_sizecols <- setNames(c("blue", "red", "yellow"),
	c("1", "2", "3"))
par(fg="black")
...
tiplabels(pie = gall_sizepp, piecol = gall_sizecols, cex=0.15)
par(fg="black")
...
par(fg="transparent")
nodelabels(pie = gall_sizeasr, piecol = gall_sizecols,cex=0.25)
par(fg="black")
legend("left", c("1", "2", "3"), pch = 21, pt.bg = c("blue", "red", "yellow"), pt.cex = 3, title = "Gall Size", cex = 1.5)
...
```{r cluster DISCRETE, fig.height=10, fig.width=20, fig.align='center'}
...
cluster.ER <- ace(cluster, S, model = "ER", type = "discrete")

clustercols <- c("red", "blue", "green", "black")
plot.phylo(S, type = "fan", cex = 2, align.tip.label = F, lwd = 3.5, label.offset = 0.1, use.edge.length = F, open.angle =  165)
cluster_DATA <- setNames(data$cluster, data$Synergus)
clusterxx <- strsplit(as.character(cluster_DATA), split = "+", fixed = T)
clusterpp <- matrix(0, length(cluster_DATA), 3, dimnames = list(names(cluster_DATA), c("0", "0+1", "1")))
for (i in 1:nrow(clusterpp)) clusterpp[i,clusterxx[[i]]]<-1/length(clusterxx[[i]])
par(fg = "transparent")
ape::tiplabels(pie = clusterpp, piecol = clustercols, cex = 0.15)
par(fg = "black")
tiplabels(pie = to.matrix(cluster, sort(unique(cluster))),piecol = clustercols, cex = 0.15)
nodelabels( node = 1:S$Nnode+Ntip(S),
    pie = cluster.ER$lik.anc, piecol = clustercols, cex=0.25)
tiplabels(pie = to.matrix(cluster, sort(unique(cluster))), piecol = clustercols, cex = 0.15)
legend("left", c("0", "0+1", "1"), pch = 21, pt.bg = c("red", "blue", "green", "black"), pt.cex = 3, title = "Clustered Galls", cex = 1.5)
